/* The 8x16 console font, for programs that draw their own text.
 *
 * A program that owns the framebuffer -- a compositor, a terminal emulator --
 * has to put characters on the screen itself, and there is no font on the
 * screen to borrow: the console's glyphs live in the kernel, on the other side
 * of the syscall boundary.  So the same font is here as well.
 *
 * These are the standard IBM VGA 8x16 glyphs, the same ones the console draws,
 * taken from the VGA BIOS QEMU ships (SeaBIOS's vgabios, LGPL-2.1).  Having
 * them in both places means a window and the console beneath it are set in the
 * same type, which is what makes the two look like one machine.
 */
