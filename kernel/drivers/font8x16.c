/* The built-in 8x16 console font.
 *
 * The console normally captures its glyphs from the graphics card, out of the
 * text-mode font memory the firmware leaves them in (vga_read_font16()).  That
 * works on a BIOS machine and nowhere else: a UEFI boot has no text mode, no
 * font memory and nothing to capture, so a kernel with no font of its own puts
 * a black screen in front of a system that is running perfectly well.
 *
 * These are the same glyphs -- the standard IBM VGA 8x16 set, taken from the
 * VGA BIOS QEMU ships (SeaBIOS's vgabios, LGPL-2.1), which is the font the
 * captured path has been drawing all along.  Extracted at
 * /usr/share/qemu/vgabios-stdvga.bin + 0x6720, 256 glyphs of 16 rows, one byte
 * per row, most significant bit leftmost.
 *
 * vga_font16() prefers a captured font when there is one, so on a BIOS machine
 * the card still supplies its own and this is the fallback.
 */

#include "types.h"

const uint8_t font8x16_builtin[256 * 16] = {
    /*   0 0x00  */
